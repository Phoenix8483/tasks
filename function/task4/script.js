...
document.querySelector('.b-12').addEventListener('click', function () {
    document.querySelector('.out-12').textContent = t12(90, 22);
})
...
const t13 = (min, max) => Math.floor(Math.random() * (max - min) + min)
...
document.querySelector('.b-18').addEventListener('click', function () {
    document.querySelector('.out-18').textContent = t18([4, 5, 6], 16);
})
...
const t20 = (obj, num) => {
    if (!Object.values(obj).includes(num)) return false;
    for (let key in j20) {
        if(j20[key] === num) return key;
    }
}
